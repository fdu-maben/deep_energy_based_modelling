...
class Sampler:
    ...
    @staticmethod
    def generate_conditional_samples(model, inp_imgs, steps=60, step_size=10, conditional_index=0,return_img_per_step=False):
        """
        Function for sampling images for a given model.
        Inputs:
            model - Neural network to use for modeling E_theta
        ...
        for _ in range(steps):
            ...
            # Part 2: calculate gradients for the current input.
            out_imgs = -model(inp_imgs)[1][...,conditional_index]
            out_imgs.sum().backward()
            inp_imgs.grad.data.clamp_(-0.03, 0.03) # For stabilizing and preventing too high gradients
...
class ConditionalGenerateCallback(pl.Callback):
        ...
        if trainer.current_epoch % self.every_n_epochs == 0:
            ...
            for i in range(10):
                ...
                for j in range(imgs_per_step.shape[1]):
                    ...
                    grid = torchvision.utils.make_grid(imgs_to_plot, nrow=imgs_to_plot.shape[0], normalize=True)
                    trainer.logger.experiment.add_image(f"conditional_generation_{i}", grid, global_step=trainer.current_epoch)
    ...
    def conditional_generate_imgs(self, pl_module, conditional_index):
        ...
        start_imgs = start_imgs * 2 - 1
        torch.set_grad_enabled(True)  # Tracking gradients for sampling necessary
        imgs_per_step = Sampler.generate_conditional_samples(pl_module.cnn, start_imgs, steps=self.num_steps, step_size=10, conditional_index = conditional_index ,return_img_per_step=True)
        torch.set_grad_enabled(False)
        pl_module.train()
        return imgs_per_step
